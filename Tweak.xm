...
- (id)initWithFrame:(CGRect)arg1
 {
    ...
    if (isEnabled) {
        ...
        UIView* _roundedCornersView = MSHookIvar<id>(self, "_roundedCornersView");
        _roundedCornersView.backgroundColor = NAV_COLOR;
...
%hook MusicActionableHeaderView

- (void)layoutSubviews {
    %orig;
    if (isEnabled) {
        [self setHeaderTitleColor:TEXT_COLOR];
    }
}
...
- (void)_setUniformTextAttributes:(NSDictionary*)arg1 {
    
    if (isEnabled) {
        NSMutableDictionary *newAttributes = [arg1 mutableCopy];
        
        [newAttributes setObject:TEXT_COLOR forKey:@"NSColor"];
...
+ (id)contextualActionsButtonDefaultTintColor { //three dots next to song
    if (isEnabled) {
        return selectedTintColor();
    }
    return %orig;
...
/*
%hook MusicCircleView //This is the pull-down arrow in the nowplaying view

-(void)layoutSubviews {
    %orig;
    if (isEnabled) {
        [self setBackgroundColor:BABY_BLUE_COLOR];
...
/*
%hook MPUTransportControlsView //Play/pause on mini media bar

-(void)layoutSubviews {
    %orig;
    
    for (UIButton* button in [self subviews]) {
        [button setTintColor:RED_COLOR];
    }
}
%end
 */
...
%hook MPUEffectView //Now Playing effect view
...
%hook SKUICrossFadingTabBarButton


- (void)setSelectionProgress:(double)sel
{
    %orig(1);
    ...
    {
        // Tab Bar unselected
        UILabel *standardTitleLabel = MSHookIvar<UILabel *>(self, "_selectedTitleLabel");
        standardTitleLabel.textColor = [UIColor grayColor];
        [self setTintColor:[UIColor grayColor]];
    }
    else if (sel == 1) {
        // Tab Bar selected
        UILabel *standardTitleLabel = MSHookIvar<UILabel *>(self, "_selectedTitleLabel");
        standardTitleLabel.textColor = selectedTintColor();
...
-(id)init {
    ...
    BOOL applicationIsEnabledInSettings = [[prefs objectForKey:[@"EnabledApps-" stringByAppendingString:[self displayIdentifier]]] boolValue];
    ...
    if ((isTweakEnabled() && applicationIsEnabledInSettings)) {
        isEnabled = isTweakEnabled(); //Is Enabled in settings
    }
    if (isTweakEnabled() && !applicationIsEnabledInSettings) {
        isEnabled = NO; //Darken
    }
    
    if (idIsEqual(@"com.apple.springboard")) { //Disable Spring Board
